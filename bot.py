...
state.setdefault("stale_run_count", 0)
state.setdefault("item_was_on_ah", True)
...
if scan_match:
    try:
        ...
        if minutes_old > STALE_THRESHOLD_MINUTES:
            state["stale_run_count"] += 1
            first_stale = not state["stale_alerted"]
            reminder_due = state["stale_run_count"] % BLOCK_REMINDER_RUNS == 0

            if first_stale or reminder_due:
